...
| **P8 = venue provision validity** | The enrolled act's mandatory Richmond-exclusive venue and pending case transfer. | Venue centralization is a substantive procedural maneuver that can produce independent legal challenges. Previously, this was analyzed solely as part of the one-object dispute (P7), but breaking it out allows the model to test if the act fails on venue grounds independently of the one-object rule. | **HB 1384 enrolled act, p. 4**: specifies the Circuit Court of the City of Richmond as the exclusive venue and directs transfer of pending cases. |

`LEGAL = P1 ∧ P2 ∧ P3 ∧ P4 ∧ P5 ∧ P6 ∧ P7 ∧ P8`

`ILLEGAL = ¬LEGAL = ¬P1 ∨ ¬P2 ∨ ¬P3 ∨ ¬P4 ∨ ¬P5 ∨ ¬P6 ∨ ¬P7 ∨ ¬P8`

An action or state is **ILLEGAL** if **at least one** of the required conditions (P1 through P8) is **FALSE** (i.e., NOT P1 OR NOT P2 OR NOT P3, etc.).
...
| **P8** | venue provision validity | `venue-ok-p` |

`legal-referral-p` is the top-level ACL2 legality predicate.
It implements legality as the conjunction of eight conditions, where:
- `first-passage-validp` = P1
- `next-election-ok-p` = P2
- `second-passage-validp` = P3
- `ninety-day-rule-ok-p` = P4
- `notice-ok-p` = P5
- `submission-clause-ok-p` = P6
- `single-object-ok-p` = P7, and
- `venue-ok-p` = P8
